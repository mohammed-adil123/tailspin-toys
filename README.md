...
The shared **Issue Triage Board** canvas (`.github/extensions/issue-triage-board/`) ranks open repository issues, highlights the three most urgent candidates with prioritization reasons, and lists the remaining issues below. Each issue can be added directly to the current Copilot session context. The canvas uses the GitHub CLI, so `gh` authentication is required when it is opened.
